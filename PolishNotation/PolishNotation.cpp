...
{
	int i = 0, s = 0, value = 0;// , size = strlen(str);
	bool check = true;
	char* str1 = (char*)str;
	...
	while (*str1 != '\0')
	{
		if (isdigit(*str1)) //strtol
		{
			value = strtol(str1, &str1, 10);
		}
		if (!(str1[i] == '(' || str1[i-1] == ')'))
		{
...
token* Polish_notation(token* TOKEN) 
{
	...
	for (token* cur = TOKEN->Head; cur!=NULL;cur=cur->next)
	{
		...
		else if (cur->type == 'o'&& (!(cur->value == -1 || cur->value == -8)))
		{
...
int find_value(token* polish_notation)
{
	stack* Notation = (stack*)malloc(sizeof(stack));
	stack_init(Notation);
	token*  pair = (token*)malloc(sizeof(token));
	token_init(pair);
	...
	for (token* cur = polish_notation->Head; cur != NULL; cur = cur->next)
	{
		...
		if (cur->type == 'o')
		{
			stack_pop(Notation, pair);
			stack_pop(Notation, pair);
			stack_push(Notation, calculate(pair->Head->value, pair->Head->next->value, cur)->Head);
			token_init(pair);
		}
	}
